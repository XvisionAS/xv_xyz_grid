...
typedef std::vector<p2t::Triangle*>	tTriangles;
...
void generate_grid(const AABB& aabb, const tPoints &points, tTriangles& out) {
	std::vector<p2t::Point*>	polyline;
	
	polyline.push_back(new p2t::Point(aabb.mMin.x * 1.1, aabb.mMin.y * 1.1));
	polyline.push_back(new p2t::Point(aabb.mMin.x * 1.1, aabb.mMax.y * 1.1));
	polyline.push_back(new p2t::Point(aabb.mMax.x * 1.1, aabb.mMax.y * 1.1));
	polyline.push_back(new p2t::Point(aabb.mMax.x * 1.1, aabb.mMin.y * 1.1));

	p2t::Point* processPoints = new p2t::Point[points.size()];
	...
	for (size_t i = 0; i < size; ++i) {
		...
		// points[i].z
		cdt->AddPoint(&processPoints[i]);
	}
	cdt->Triangulate();
	auto triangles = cdt->GetTriangles();
	
	
	delete [] processPoints;
}
...
int main(int ac, char **av) {
	...
	for (size_t arg = 0; arg < inputs.size(); arg++) {
...

		{
			...
			generate_grid(aabb, points_result, triangles);
		}
